...
func _randomise(sound):
	...
	var dloc = sound.get_parent().dlocs[sound.get_index()]
	var newvol = (dvol + rand_range(-volume_range,volume_range))
	var newpitch = (dpitch + rand_range(-pitch_range,pitch_range))
	var newloc = (dloc + Vector3(randf() * scatter_range, randf() * scatter_range, randf() * scatter_range))
